...
| PUT /api/admin/config            | 部分更新：undefined=不变、""=清除（回退 env）、非空=设置；写 D1 app*config 并失效 KV 缓存，**即时生效免 redeploy**（覆盖 aiSummary.*/embedModel/embedAutogen/github.\_/r2.\*）                                                                                                                                                                                                  | admin               |
