...
| PUT /api/admin/config            | 部分更新：undefined=不变、""=清除（回退 env）、非空=设置；写 D1 app_config 并失效 KV 缓存，**即时生效免 redeploy**（覆盖 aiSummary._/embedModel/embedAutogen/github._/r2.\*）                                                                                                                                                                                                   | admin               |
...
| POST /api/export/trigger         | 触发 GitHub repository_dispatch('hyacine:export')；未配置 github → 400 {dispatched:false,error}                                                                                                                                                                                                                                                                                 | admin               |
